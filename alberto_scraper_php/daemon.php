...
while (!$done){  
	...
	$query = "SELECT app FROM play_apps_index where index = FALSE order by app DESC LIMIT '$limit' for Update";
	$results = pg_query($query); 
	...
	$query = "UPDATE play_apps_index SET index = TRUE WHERE app in (SELECT app FROM play_apps_index where index = FALSE order by app DESC LIMIT '$limit')";
	pg_query($query);
...
function upsert($app) 
{
	...
	$query = "UPDATE rest_api_application SET 
		...
		updated = ".time()."  WHERE app_id = '".$app['app_id']."'  ";
	...
	$query = "INSERT INTO rest_api_application (
		...
		".time().",
		".time()." WHERE NOT EXISTS (SELECT 1	 FROM rest_api_application WHERE app_id = '".$app['app_id']."')";		
	...
	$query = "UPDATE rest_api_developer SET 
		...
		developer_email = '".$app['developer_mail']."'  WHERE developer_id = '".$app['developer_id']."' ";
	...
	$query = "INSERT INTO rest_api_developer (
			...
			'".$app['developer_name']."',
			'".$app['developer_web']."',
			'".$app['developer_mail']."' WHERE NOT EXISTS (SELECT 1 FROM rest_api_developer WHERE developer_id = '".$app['developer_id']."' ) ";
...
$total = microtime(true); 
	...
	switch ($language) {
        case "en":
        	...
        	setlocale (LC_TIME, 'en_US.UTF-8'); //mac
			$formato = "%B %d, %Y";	
			$storefront_id = 1002;
        ...
        case "es":
        	...
        	setlocale (LC_TIME, 'es_ES.UTF-8'); //mac
			$formato = "%d de %B de %Y";		
			$storefront_id = 1001;
		...
		*/
        ...
        if (!empty($recommendations)) {
            foreach ($recommendations as $recomendation) {
            	...
            	$related = $recomendation->attr('data-docid');  
            	//create file with apps
            	$f = fopen('index.txt','a+');
				fwrite($f, $related.PHP_EOL);
				fclose($f);
        ...
        if (file_exists('index.txt')) {
			$lines = array_unique(file('index.txt'));
			$f = fopen('index.txt','w+');
			fwrite($f, join('',$lines));
			fclose($f);
		}
        ...
        $app['developer_web'] = $developer_web;
        $app['developer_mail'] = $developer_mail;     
        $app['deveoper_privacy'] = $developer_privacy;    
