...
function upsert($app) 
{
	...
	$query = "UPDATE rest_api_appdetails SET 
		...
		updated = ".time()."  WHERE (app_id = '".$app['app_id']."' AND language_code = '".$app['language_code']."') ";
	...
	$query = "INSERT INTO rest_api_appdetails (
		...
		".time().",
		".time()." WHERE NOT EXISTS (SELECT 1 FROM rest_api_appdetails WHERE (app_id = '".$app['app_id']."' AND language_code = '".$app['language_code']."') )";
...
$total = microtime(true); 
    ...
    try {
        ...
        $lan = $page->find('.app-translated-desc')->text();
		
		if (!empty($lan)){
			$app=array('estado'=> 'Translation');
			echo 'App:: '.$app_id.'Not in '.$language;
            return $app;	 
		}
        ...
        $app['language_code'] = $language_code;
