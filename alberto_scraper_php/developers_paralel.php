...
// ALTER TABLE rest_api_developer ADD COLUMN index boolean DEFAULT false;
// ALTER TABLE rest_api_developer DROP COLUMN index RESTRICT;
...
while (!$done){  
	...
	$query = "SELECT developer_id FROM rest_api_developer where index = FALSE order by developer_id ASC LIMIT '$limit' for Update";
	$results = pg_query($query); 
	...
	$query = "UPDATE rest_api_developer SET index = TRUE WHERE developer_id in (SELECT developer_id FROM rest_api_developer where index = FALSE order by developer_id ASC LIMIT '$limit')";
	pg_query($query);
...
echo 'Developers Inactive: '.$developers_deleted.PHP_EOL;
